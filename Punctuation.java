 ...
 * 
 * searchIt()
 * 	- D&I paragrpah as getPunctuationWords()
 * 	- call printWords()
 * 
 * getPunctuationWords()
 * 	- D&I paragraph as the pre-determined value
 * 	- run a for loop that runs through the indices of the string
 * 	- Seperate a substring that is each word
 * 	- run checkForPunctuation(substr) to check if the substring has punctuation in it
 * 	- if it does then add it to the punctuated paragraph
 * 	- return the punctuated paragraph
 * 
 * checkForPunctuation(substr)
 * 	- D&I a string that has all of the punctuation in it
 *  - check if the substr has any of the characters in it using a nested for loop
...
public class Punctuation
{
	...
	public String getPunctuationWords()
	{
		...
		"ages, that mankind will move forward towards its goal. " + "But I take up my task with buoyancy and hope. I feel sure that our cause " +
		...
		String punctuatedParagraph = new String("");
		int startSubStr = 0;
		...
		for(int i = 0; i < paragraph.length(); i ++)
		{
			...
			{
				String newSub = paragraph.substring(startSubStr, i);
				if(checkForPunctuation(newSub))
					punctuatedParagraph += newSub;
	...
	public void printWords(String paragraph)
	{
